...
def backtest(ticks: np.ndarray, settings: dict):
    ...
    for k, t in enumerate(ticks):
        ...
        if did_trade:
            ...
            actual_balance = actual_balance + pnl + fee_paid
            apparent_balance = actual_balance * ss['balance_pct']
            
            drawdown = -(actual_balance - max_balance)/max_balance
            if drawdown > max_drawdown:
                max_drawdown = drawdown
                
            if actual_balance > max_balance:
                max_balance = actual_balance
            ...
            trades.append({'trade_id': k, 'side': trade_side, 'type': trade_type, 'price': price,
                           ...
                           'progress': progress, 'max_drawdown': max_drawdown})
            closest_long_liq, closest_shrt_liq = 1.0, 1.0
            if pos_size > 0.0:
            ...
            line = f"\r{progress:.3f} "            
            line += f"actual_bal {actual_balance:.2f}/{settings['starting_balance']} "
...
#             line += f"pos size {pos_size:.4f} "
            line += f"adg {adg-1:.4f} "
            line += f"max drawdown {max_drawdown:.4f} "
            print(line, end=' ')
...
def jackrabbit_wrap(ticks: [dict], backtest_config: dict) -> dict:
    ...
    result = {
        ...
        'max_drawdown': trades[-1]['max_drawdown'],
...
async def prep_backtest_config(config_name: str):
    backtest_config = hjson.load(open(f'{base_path}/backtest_configs/{config_name}.hjson'))
...
fitting_params = ['balance_pct', 'entry_qty_pct', 'ddown_factor', 'ema_span', 'ema_spread', 
                  'grid_coefficient', 'grid_spacing', 'leverage', 'stop_loss_liq_diff', 
...
def backtest_pso0(x, ss, ticks):
    ...
    stop_loss_liq_diff_price, stop_loss_pos_price_diff_price, stop_loss_price = 0.0, 0.0, 0.0
    actual_balance = ss['starting_balance']
    apparent_balance = actual_balance * x[0]
    ...
    if ss['inverse']:
        ...
        min_entry_qty_f = calc_min_entry_qty_linear
        long_liq_price_f = lambda bal, psize, pprice: \
            binance_calc_cross_long_liq_price(bal, psize, pprice, x7)
        shrt_liq_price_f = lambda bal, psize, pprice: \
            binance_calc_cross_shrt_liq_price(bal, psize, pprice, x7)
    ...
    for k, t in enumerate(ticks):
        ...
        if t[1]:
            ...
            if pos_size == 0.0:
                # create long pos
                if ss['do_long']:
                    price = calc_no_pos_bid_price(ss['price_step'], x[4], ema, ob[0])
                    if t[0] < price and ss['do_long']:
                        did_trade = True
                        qty = min_entry_qty_f(ss['qty_step'], ss['min_qty'], ss['min_cost'],
                                              x[1], x7, apparent_balance,
                                              price)
                        trade_type, trade_side = 'entry', 'long'
                        pnl = 0.0
                        fee_paid = -cost_f(qty, price) * ss['maker_fee']
...
#                     print('\nlong liquidation')
                ...
                if t[0] <= pos_price:
                    ...
                    # shrt stop loss
                    did_trade = True
                    qty = calc_pos_reduction_qty(ss['qty_step'], x[14],
                                                 pos_size)
                    price = stop_loss_price
                    trade_type, trade_side = stop_loss_type, 'shrt'
                    pnl = shrt_pnl_f(pos_price, price, qty)
            ...
            if pos_size == 0.0:
                # create shrt pos
                if ss['do_shrt']:
                    price = calc_no_pos_ask_price(ss['price_step'], x[4], ema, ob[1])
                    if t[0] > price:
                        did_trade = True
                        qty = -min_entry_qty_f(ss['qty_step'], ss['min_qty'], ss['min_cost'],
                                               x[1], x7,
...
#                     print('\nshrt liquidation')
        ...
        if did_trade:
            ...
            min_entry_qty = min_entry_qty_f(
                ss['qty_step'], ss['min_qty'], ss['min_cost'], x[1], x7,
                apparent_balance, t[0]
            )
            if apparent_balance * x7 < cost_f(min_entry_qty, t[0]):
#                 print('\nself liquidated')
                return 0.0
            if pos_size == 0.0:
                liq_price = 0.0
            ...
            closest_long_liq, closest_shrt_liq = 1.0, 1.0
            if pos_size > 0.0:
                stop_loss_liq_diff_price = liq_price * (1 + x[8])
                stop_loss_pos_price_diff_price = pos_price * (1 - x[9])
                stop_loss_price = 0.0
                reentry_price = min(
                    ob[0],
                    calc_long_reentry_price(ss['price_step'], x[6],
                                            x[5], apparent_balance,
                                            pos_margin_f(x7, pos_size, pos_price),
                                            pos_price)
                )
                reentry_price = max(ss['price_step'], reentry_price)
                min_qty_ = min_qty_f(ss['qty_step'], ss['min_qty'], ss['min_cost'], reentry_price)
                reentry_qty = calc_reentry_qty(ss['qty_step'],
                                               x[2],
                                               min_qty_,
                                               max_pos_size_f(x7, apparent_balance,
                                                              reentry_price),
                                               pos_size)
                if reentry_qty < min_qty_:
                    reentry_price = ss['price_step']
                ...
                reentry_price = max([
                    ss['price_step'],
                    ob[1],
                    calc_shrt_reentry_price(ss['price_step'], x[6],
                                            x[5], apparent_balance,
                                            pos_margin_f(x7, pos_size, pos_price),
                                            pos_price)
                ])
...
#     print(f"actual_bal {trades[-1]['actual_balance']:.2f}/{settings['starting_balance']} ")
#     print(f"adg {trades[-1]['average_daily_gain']-1:.4f} ")
#     print(f"max drawdown {trades[-1]['max_drawdown']:.4f} ")
    return -actual_balance

async def backtest_pso(config_name, n_particles=10, iters=100, n_processes=None):
    if n_processes == None:
        n_processes = os.cpu_count()
        
    global settings
    settings = await prep_backtest_config(config_name)
    candidate = get_new_candidate(
        settings['ranges'],
        {k_: 0.0 for k_ in settings['ranges']},
        m=1.0
    )
...
#     global ticks
    ...
    def f(x):
        ...
        j = [backtest_pso1(x[i]) for i in range(n_particles)]
        return np.array(j)
    
    options = {'c1': 1.0, 'c2': 1.0, 'w': 1.0}

    # Call instance of GlobalBestPSO
    optimizer = ps.single.GlobalBestPSO(n_particles=n_particles, dimensions=15, options=options, bounds=bounds)

    # Perform optimization
    stats = optimizer.optimize(f, iters=iters, n_processes=n_processes)
...
def array_to_live_settings_file(exchange: str, candidate, config) -> dict:
    ...
    live_result_path = get_live_result_path(settings)
